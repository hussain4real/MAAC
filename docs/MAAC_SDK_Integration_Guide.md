...
Current **API contract version: v0.0.1**; **SDK packages: v0.1.0** (a backward-compatible
minor that adds the async/polling/streaming/webhook runtime modes — a v0.0.1 client still
works). `GET /api/v1/sdk` advertises a `capabilities` block so a client can detect the
supported runtime modes before using them.

| SDK / stack                         | Version | Status        | Notes |
|-------------------------------------|---------|---------------|-------|
| PHP SDK (`milaha/maac-sdk`)         | 0.1.0   | ✅ Supported  | PHP ≥ 8.2, ext-curl. Default cURL transport. |
| TypeScript SDK (`@maac/sdk`)        | 0.1.0   | ✅ Supported  | Node ≥ 18 (global `fetch`); zero dependencies. |
...
| Remote HTTP & MCP connector tools   | —       | 🗓️ Coming soon | Today only client-side + MAAC-hosted tools execute. |
