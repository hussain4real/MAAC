...
## Server-side tools (remote HTTP & MCP connectors)
...
- **Server-side** — MAAC runs it itself and never calls back to your app:
  ...
  - **Remote HTTP** tools (MAAC calls an allowlisted external endpoint), and
...
foreach ($manifest->agents as $agent) {
    foreach ($agent->serverTools as $tool) {
        // $tool['name'], $tool['execution_mode'] (hosted|http|connector), $tool['description']
...
for (const agent of manifest.agents) {
  for (const tool of agent.serverTools) {
    // tool.name, tool.executionMode ('hosted' | 'http' | 'connector'), tool.description
