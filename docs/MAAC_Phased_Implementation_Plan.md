...
- [x] Package reusable SDK client code rather than copying request logic into each reference app. *(`maac/sdk` for PHP, `@qatar-navigation-milaha/sdk` for TypeScript.)*
...
> **Status: ✅ Complete — 8A ✅, 8B ✅.** Phase 8 closes the last execution-mode gap and formalizes MAAC's own administration model. **Phase 8A** (branch `feature/maac-phase-8a-db-tools`) implemented and governed the read-only database (`db`) execution mode, so the runtime now supports every BRS execution mode with no remaining `unsupported_execution_mode`; it should be used only for approved analytics/reporting use cases where a client-side, remote HTTP, MCP, or knowledge tool is not the safer fit. **Phase 8B** (branch `feature/maac-phase-8b-admin-rbac`) added the dedicated MAAC platform-administration RBAC layer (Spatie-backed global roles/permissions, Super Admin override, audited grant management, break-glass, access review, SSO role mapping) — making clear that platform administration, approvals, and operational sign-off are MAAC-admin responsibilities, held above the team/project-scoped tenant RBAC and never granted to tenant users by default.

### Phase 8A: Governed Read-Only Database Tools

> **Status: ✅ Complete** — branch `feature/maac-phase-8a-db-tools`. The `db` execution mode that previously dead-ended at `unsupported_execution_mode` is now a real, governed read-only database capability — and it is the last execution mode the runtime did not support, so **there are no remaining `unsupported_execution_mode`s**. A new **read-only data source registry** (`data_sources` table + `DataSource` model, `DbConnectionType` + `DataSourceStatus` enums) records a source's owner, business purpose, connection type, environment availability, sensitivity, approved query surface (`allowed_relations`), and result/freshness caps. MAAC **never persists a connection string, username, or password**: a source references an approved, ops-provisioned read-only Laravel connection *by name* (validated against the `maac.runtime.db.allowed_connections` allowlist, which blocks pointing at MAAC's own operational DB), and any injected credential is resolved from the **secrets vault** at query time (`DataSourceConnection`, vault kind `database`). `db` tool contracts gain `data_source_id` + `db_config` (parameterized SELECT template, input bindings, minimized output columns, row limit, freshness). The **`DbToolExecutor`** (routed from `AgentRunner::handleToolCall`, alongside hosted/http/connector/knowledge) enforces source active+environment+not-stale, then `DbQueryGuard` (single read-only SELECT/CTE only, blocked write/DDL/transaction keywords as whole words over a literal-stripped copy, no comments/stacked statements, allowlisted relations, named-parameter binding — never interpolation), runs on the read-only connection via a row-capped cursor, projects to the minimized columns, enforces a result-size limit, validates the output schema, and **redacts the stored copy** (the LLM still sees raw). Every failure is a controlled code (`db_source_misconfigured`/`_unavailable`, `db_unsafe_query`, `db_connection_failed`, `db_query_failed`, `db_too_many_rows`, `db_result_too_large`, `db_stale`, `db_invalid_output`). A sensitive source is gated behind a new `ApprovalType::DataSourceAccess` (Draft → approve → Active); a `db` tool that requires approval starts Draft and the runtime blocks it until granted; `ApprovalGate` blocks agent publication while an assigned `db` tool's source is disabled/unavailable; the approval review surfaces the connection type, query surface, and query for egress review. The SDK manifest advertises `db` under `capabilities.tool_execution_modes.server_side` and in per-agent `server_tools`; the SDK docs page + integration guide move read-only DB to **Supported**. A new **Data Sources** console page (Integrate group) registers/manages sources with an explicit "governed reporting data, not application-owned transactional access" warning, and the tool form gained a `db` config section. Verified: **921 Pest tests at 100 % line coverage**, PHPStan L7, Pint, ESLint, Prettier, `tsc` (frontend + SDK), 30 Node tests, `maac:sdk-fixtures --check`, and `npm run build` all green — plus a live Chrome walkthrough (Data Sources page, a live confidential-source registration gated to Draft + approval, a published agent executing the `db` tool through the served runtime returning 5 minimized reporting rows with a trace + cost, the run-detail trace, and the SDK docs Supported row) with no console errors.
...
- [x] Add read-only data source registration with owner, environment availability, sensitivity, business purpose, connection type, and approval status.
- [x] Store database credentials through the secrets vault only; never persist plaintext connection strings, usernames, passwords, certificates, or tokens. *(MAAC stores only an approved connection-name reference; any credential is vault-resolved at query time.)*
- [x] Restrict connections to approved read-only replicas, materialized views, reporting schemas, or curated database views; block unrestricted production database access. *(`maac.runtime.db.allowed_connections` allowlist + `DbConnectionType`.)*
- [x] Add query governance: allowlisted views/tables, statement type enforcement, blocked keywords, parameter binding, row limits, timeout limits, result-size limits, and explain-plan or dry-run validation where supported. *(`DbQueryGuard` + executor; statement timeout is enforced by the ops-provisioned read-only connection and recorded per source.)*
- [x] Add a `db` tool configuration model that maps a tool contract to an approved data source, query template or query-builder definition, input bindings, output schema, redaction rules, and freshness expectations.
- [x] Execute `db` tools through a dedicated runtime executor that validates inputs, binds parameters safely, runs the query with a read-only connection, validates outputs, redacts stored results, and returns only schema-approved fields to the LLM.
- [x] Add approval gates for production `db` tools, covering data owner approval, security review, query surface, sensitivity, retention, and maximum result scope.
- [x] Add runtime safeguards for unauthorized data sources, disabled sources, unsafe query shapes, timeout, too many rows, oversized payloads, invalid output schema, stale replicas, and connection failures.
- [x] Surface read-only data sources and `db` tool configuration in the console with clear warnings that this mode is for governed reporting data, not application-owned transactional access.
- [x] Update the SDK manifest, SDK docs page, integration guide, and compatibility matrix so external apps can distinguish `db` tools as MAAC-executed server-side tools.
- [x] Add focused feature, runtime, policy, approval, and E2E tests proving successful `db` execution plus every controlled failure path.
- [x] Keep `composer ci:check`, `php artisan test --coverage --min=100 --compact`, `maac:sdk-fixtures --check`, SDK tests, and browser smoke coverage green.
...
> **Status: ✅ Complete** — branch `feature/maac-phase-8b-admin-rbac`. MAAC now has a dedicated, global platform-administration RBAC layer backed by `spatie/laravel-permission`, distinct from (and layered above) the team/project-scoped tenant RBAC — a user with no platform role is a pure tenant user and can never reach the platform-admin surfaces. **Foundation:** Spatie runs in non-teams mode (`config/permission.php`); the `User` adds `HasRoles` + a `HasPlatformAccess` concern (`isPlatformSuperAdmin`/`isPlatformAdministrator`/`hasPlatformPermission` via `can()`); a `Gate::before` override in `AppServiceProvider` grants the **Super Admin** an unrestricted bypass (composing with the spatie `permission`/`role` middleware, whose `canAny()` goes through the gate). **Catalogue:** `App\Enums\PlatformRole` (Super Admin, Platform Admin, Security Reviewer, Auditor, Support Operator, Release Manager, Read-Only Observer) → `App\Enums\PlatformPermission` (33 granular `domain.action` permissions across users/teams/applications/projects/agents/tools/models/credentials/approvals/quotas/runs/audits/webhooks/SDK/incidents/settings/access-review/break-glass), seeded by `Database\Seeders\PlatformRbacSeeder` (idempotent; bootstraps the `config('maac.platform.super_admins')` emails so the platform is never left without an administrator — empty by default in production). **Mapping:** SSO `group_role_mappings` gained a `platform_role` field (`SsoConnection::resolvePlatformRoles`); `SsoUserResolver` idempotently assigns mapped platform roles on login — a tenant gets none unless an IdP group is explicitly mapped. **Audited management:** a `platform_access_grants` ledger + `PlatformAccessManager` keep Spatie and the governance trail in sync across grant / break-glass / revoke / certify / SSO-sync / expiry, each writing a `platform_access.*` audit event; a new admin-only **Access Control** console page (`ConsoleController::accessControl` + `PlatformAccessController` write actions, gated by `permission:` middleware; the two most sensitive grants — Super Admin and break-glass — are Super-Admin-only in `StorePlatformAccessRequest`) assigns/revokes/certifies and surfaces the access-review work lists. **Break-glass + access review:** time-boxed emergency grants (TTL clamped to `config('maac.platform.break_glass')`) that auto-expire, plus `maac:review-platform-access` (scheduled daily) which revokes elapsed break-glass and reports grants needing re-certification and stale (inactive) admin accounts. **Frontend:** `auth.platform` (roles/permissions/super-admin) is shared to Inertia, and the Access Control nav item is gated on the real `users.view` permission (not the persona mock). Verified: **957 Pest tests (954 passed, 3 env-gated live-provider skips) at 100 % line coverage**, PHPStan L7, Pint, ESLint, Prettier, `tsc` (frontend + SDK), 30 Node tests, `maac:sdk-fixtures --check` (SDK contract unchanged — this phase hardens the console, not the SDK surface), and `npm run build` all green — plus a live Chrome walkthrough of the Access Control page (grant a role, break-glass, the access review, denied-tenant) with no console errors.
...
- [x] Install and configure `spatie/laravel-permission` as the dedicated MAAC admin RBAC layer, including guard configuration, migrations, cache handling, and team/tenant boundary decisions. *(Non-teams mode, `web` guard; `HasRoles` on `User` with the `HasTeams::teams insteadof HasRoles` resolution.)*
- [x] Define and seed MAAC platform roles such as Super Admin, Platform Admin, Security Reviewer, Auditor, Support Operator, Release Manager, and Read-Only Observer. *(`PlatformRole` + `PlatformRbacSeeder`.)*
- [x] Define granular permissions for platform users, teams, registered applications, projects, agents, tools, models, credentials, approvals, quotas, runs, audits, webhooks, SDK/package operations, and system settings. *(`PlatformPermission` — 33 permissions.)*
- [x] Add a top-level Super Admin override for MAAC-owned platform administration while keeping tenant/application data access controlled by the owning application and approved tool contracts. *(`Gate::before` super-admin bypass.)*
- [x] Map existing authenticated users, team membership, and SSO/IAM claims into MAAC roles and permissions without granting tenant users platform-admin privileges by default. *(Config bootstrap + `SsoConnection::resolvePlatformRoles` / `SsoUserResolver::syncSsoRole`; tenants get no platform role by default.)*
- [x] Protect MAAC management routes, controller actions, policies, API endpoints, Inertia props, and navigation items with Spatie-backed permissions. *(`permission:` middleware on the access-control routes, `Gate::before`, the augmented gate checks, `auth.platform` Inertia share, and the permission-gated nav item.)*
- [x] Add an admin role/permission management UI for MAAC admins to assign, revoke, inspect, and audit platform access. *(The Access Control console page.)*
- [x] Record role and permission changes in the audit log, including actor, target user, previous access, new access, reason, and affected tenant/application scope. *(`platform_access.*` audit events with actor/target/role/reason metadata.)*
- [x] Add break-glass and access-review controls for emergency admin access, permission drift detection, stale admin accounts, and periodic access certification. *(Time-boxed break-glass grants + `maac:review-platform-access`.)*
- [x] Update the BRS/admin authorization notes to make clear that pilot readiness, release baselines, platform approvals, and operational sign-off are MAAC-admin responsibilities, not tenant-user workflows. *(`docs/MAAC_BRS(1).md` + `docs/MAAC_Platform_Administration.md`.)*
- [x] Add feature, policy, authorization, audit, and browser tests covering every MAAC admin role, denied tenant access, Super Admin override, SSO role mapping, and permission-management flow. *(`tests/Feature/Platform/` — 33 tests at 100 % coverage; browser coverage as Inertia page-render assertions plus a live Chrome walkthrough, matching every prior phase.)*
- [x] Run and record the final control gate: `composer ci:check`, `php artisan test --coverage --min=100 --compact`, authorization matrix tests, browser smoke for admin access, and SDK fixture checks. *(All green; see the status summary.)*
