...
> **Status: In progress — 8A ✅ Complete, 8B Planned.** Phase 8 turns the completed MAAC platform into a production-pilot-ready release while closing the only documented execution-mode gap that remained after Phase 6G: read-only database (`db`) tools. **Phase 8A is now complete** (branch `feature/maac-phase-8a-db-tools`) — the `db` execution mode is implemented and governed, so the runtime now supports every BRS execution mode with no remaining `unsupported_execution_mode`. Phase 8A should be used only for approved analytics/reporting use cases where a client-side, remote HTTP, MCP, or knowledge tool is not the safer fit. Phase 8B (production-pilot readiness & release baseline) should be run before any broad rollout, even if `db` tools are deferred for the first pilot.
